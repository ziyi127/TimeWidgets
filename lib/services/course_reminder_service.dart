  ...
  void _updateTimer() {
    ...
    if (_settings?.enableCourseReminder ?? false) {
      ...
      _checkTimer = Timer.periodic(const Duration(minutes: 1), (timer) {
        unawaited(_checkUpcomingCourses());
      });
    ...
    try {
      ...
      for (final course in timetable.courses) {
        ...
        if (minutesUntilStart == 5 && !course.isCurrent) {
          unawaited(_sendCourseReminder(course, ReminderType.upcoming));
        }

        // 如果课程刚刚开始（0分钟），发送提醒
        if (minutesUntilStart == 0 && !course.isCurrent) {
          unawaited(_sendCourseReminder(course, ReminderType.start));
        }

        // 如果课程即将结束（1分钟前），发送提醒
        if (minutesUntilEnd == 1 && course.isCurrent) {
          unawaited(_sendCourseReminder(course, ReminderType.ending));
        }

        // 如果课程刚刚结束（0分钟），发送提醒
        if (minutesUntilEnd == 0 && course.isCurrent) {
          unawaited(_sendCourseReminder(course, ReminderType.end));
        }
      }
    } catch (e) {
  ...
  Future<void> _sendCourseReminder(Course course, ReminderType type) async {
    ...
    switch (type) {
      case ReminderType.upcoming:
        reminderMessage =
            '课程即将开始: ${course.subject} 将在5分钟后开始，上课时间 ${course.time}，地点 ${course.classroom}';
        break;
      case ReminderType.start:
        reminderMessage =
            '课程开始: ${course.subject} 已经开始，上课时间 ${course.time}，地点 ${course.classroom}';
        break;
    ...
    if (type == ReminderType.start &&
        (_settings?.showNotificationOnClassStart ?? false)) {
      // 可以添加开始提醒的特殊处理
    }

    if (type == ReminderType.end &&
        (_settings?.showNotificationOnClassEnd ?? false)) {
      // 可以添加结束提醒的特殊处理
    }
  }
}
