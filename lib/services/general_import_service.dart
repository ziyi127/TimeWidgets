...
class GeneralImportService {
  ...
  static GeneralImportResult _heuristicParseJson(dynamic json) {
    ...
    _traverseJson(json, (key, value) {
      ...
      if (_isCourseList(key, value)) {
        _parseCourses(value, courses);
      }
      // 2. 尝试识别时间表/时间点
      else if (_isTimeLayoutOrSlotList(key, value)) {
        _parseTimeLayoutsAndSlots(value, timeLayouts, timeSlots);
      }
      // 3. 尝试识别课表/日程
    ...
    _traverseJson(json, (key, value) {
      if (_isScheduleList(key, value)) {
        _parseSchedules(value, schedules, courses, timeLayouts, timeSlots);
      }
    });
  ...
  static void _traverseJson(dynamic data, Function(String key, dynamic value) onVisit) {
    if (data is Map) {
  ...
  static bool _itemsLookLikeCourses(List list) {
  ...
  static bool _itemsLookLikeTimeSlots(List list) {
    if (list.isEmpty) return false;
    final first = list.first;
    if (first is! Map) return false;
    // 必须包含时间字段
  ...
  static bool _itemsLookLikeTimeLayouts(List list) {
    if (list.isEmpty) return false;
    final first = list.first;
    if (first is! Map) return false;
    // 包含时间点列表
  ...
  static bool _itemsLookLikeSchedules(List list) {
    if (list.isEmpty) return false;
    final first = list.first;
    if (first is! Map) return false;
    // 包含课程列表
    return first.keys.any((k) => k.toLowerCase().contains('course') || k.toLowerCase().contains('class'));
  }
  
  static bool _itemsLookLikeDailyCourses(List list) {
    if (list.isEmpty) return false;
    final first = list.first;
    if (first is! Map) return false;
  ...
  static bool _hasField(Map map, List<String> candidates) {
  ...
  static void _parseCourses(List list, List<CourseInfo> result) {
  ...
  static void _parseTimeLayoutsAndSlots(List list, List<TimeLayout> layouts, List<TimeSlot> slots) {
  ...
  static TimeSlot? _parseTimeSlot(Map item, int index) {
     ...
     if (_itemsLookLikeDailyCourses(list)) {
       ...
       for (final item in list) {
         if (item is Map) {
           ...
           if (dc != null) dailyCourses.add(dc);
         }
       }
       if (dailyCourses.isNotEmpty) {
         schedules.add(Schedule(
           ...
           name: '导入课表',
           triggerRule: const ScheduleTriggerRule(weekDay: 1), // 默认周一
       ...
       for (final item in list) {
         ...
         item.forEach((k, v) {
           if (v is List && _itemsLookLikeDailyCourses(v)) {
             for (final courseItem in v) {
               if (courseItem is Map) {
                 final dc = _parseDailyCourse(courseItem, courses, slots);
                 if (dc != null) scheduleCourses.add(dc);
               }
             }
           }
         });
  ...
  static DailyCourse? _parseDailyCourse(Map item, List<CourseInfo> courses, List<TimeSlot> slots) {
    // 解析 Course
    ...
    if (timeSlotId == null) {
       ...
       if (startTime != null) {
         ...
         if (slot.id.isNotEmpty) timeSlotId = slot.id;
       }
    }
  ...
  static String? _getString(Map map, List<String> keys) {
  ...
  static int? _getInt(Map map, List<String> keys) {
    for (final key in keys) {
      for (final mapKey in map.keys) {
  ...
  static String? _getTimeString(Map map, List<String> keys) {
