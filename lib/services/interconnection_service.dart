...
class InterconnectionService {
  ...
  static final InterconnectionService _instance =
      InterconnectionService._internal();
  ...
  Stream<List<DiscoveredDevice>> get pairedDevicesStream =>
  ...
  Future<void> _loadPairedDevices() async {
    ...
    if (jsonList != null) {
      ...
      _pairedDevices.addAll(
        jsonList.map((e) =>
            DiscoveredDevice.fromJson(jsonDecode(e) as Map<String, dynamic>)),
      );
  ...
  Future<void> reconnectPairedDevices() async {
    ...
    Logger.i(
        'Attempting to reconnect to ${_pairedDevices.length} paired devices...');
    ...
    for (final device in devicesToSync) {
      try {
        ...
        Logger.w(
            'Failed to auto-sync with paired device ${device.name} (${device.ip}): $e');

        // Try to recover IP via Auth Token Broadcast
        if (device.authToken != null) {
          Logger.i(
              'Attempting IP recovery for ${device.name} via Auth Token...');
          ...
          final updatedIndex = _pairedDevices.indexWhere(
              (d) => d.name == device.name && d.authToken == device.authToken);

          if (updatedIndex != -1) {
            final updatedDevice = _pairedDevices[updatedIndex];
            // Even if IP is same, maybe the device just came online, so retry anyway
            Logger.i(
                'Retrying sync with ${updatedDevice.name} at ${updatedDevice.ip} after recovery window...');
            try {
  ...
  Future<void> _handleBroadcastPacket(RawSocketEvent event) async {
      ...
      try {
        ...
        final device = DiscoveredDevice(
          ...
          authToken: data['authToken']
              as String?, // Capture auth token from broadcast if present
        );

        // Check if this matches a paired device but with different IP
        final pairedIndex = _pairedDevices.indexWhere(
          (d) => d.name == device.name && d.authToken == device.authToken,
        );
        ...
        if (pairedIndex != -1) {
          ...
          if (pairedDevice.ip != device.ip) {
            Logger.i(
                'IP changed for paired device ${device.name}. Updating from ${pairedDevice.ip} to ${device.ip}');
        ...
        if (index != -1) {
          ...
          Logger.i(
              'Discovered new device: ${device.name} at ${device.ip}:${device.port}');
        }
        _devicesController.add(List.from(_discoveredDevices));
      } catch (e) {
  ...
  Future<void> connectAndSync(DiscoveredDevice device,
      {int retries = 2}) async {
    Socket? socket;
    try {
      ...
      if (socket == null) {
        ...
        socket.add(TransferProtocol.createPacket(
            PacketType.handshake, handshakePayload));
      }
      ...
      if (retries > 0) {
        Logger.i(
            'Retrying sync with ${device.name} ($retries attempts left)...');
  ...
  Future<void> startBroadcasting() async {
    ...
    try {
      ...
      _authRecoverySocket = await RawDatagramSocket.bind(
          InternetAddress.anyIPv4, _authRecoveryPort);
      _authRecoverySocket!.listen((event) {
        if (event == RawSocketEvent.read) {
          ...
          try {
            final message = utf8.decode(datagram.data);
            final data = jsonDecode(message) as Map<String, dynamic>;
            if (data['type'] == 'auth_recovery' &&
                data['authToken'] == _authToken) {
              Logger.i(
                  'Received auth recovery request from Master. Broadcasting presence...');
    ...
    socket.listen(
      (data) async {
        ...
        while (true) {
          ...
          final headerView = ByteData.view(Uint8List.fromList(
                  buffer.sublist(0, TransferProtocol.headerLength))
              .buffer);
  ...
  Future<void> _processPacket(
      Socket socket, Map<String, dynamic> packet) async {
    ...
    if (typeStr == 'handshake') {
      ...
      if (data['timetable'] != null) {
        final timetableData =
            TimetableData.fromJson(data['timetable'] as Map<String, dynamic>);
