...
class _EnhancedWidgetWrapperState extends State<EnhancedWidgetWrapper> {
  ...
  Widget build(BuildContext context) {
    ...
    final defaultBgColor = colorScheme.surfaceContainerHighest.withValues(alpha: 0.6 * componentOpacity);
    final effectiveBgColor = widget.backgroundColor != null 
        ? widget.backgroundColor!.withValues(alpha: widget.backgroundColor!.a * componentOpacity)
        : defaultBgColor;
    
    final effectiveBorderRadius = widget.borderRadius ?? BorderRadius.circular(
      ResponsiveUtils.getBorderRadius(width),
    );
    final effectivePadding = widget.padding ?? EdgeInsets.all(ResponsiveUtils.value(12));

    Widget content = AnimatedContainer(
      duration: const Duration(milliseconds: 200),
      curve: Curves.easeInOut,
      decoration: BoxDecoration(
        color: enableGradients ? null : (_isHovered && widget.isInteractive 
            ? Color.alphaBlend(colorScheme.primary.withValues(alpha: 0.08), effectiveBgColor)
            : effectiveBgColor),
        gradient: enableGradients ? LinearGradient(
          begin: Alignment.topLeft,
          end: Alignment.bottomRight,
          colors: [
            _isHovered && widget.isInteractive 
                ? Color.alphaBlend(colorScheme.primary.withValues(alpha: 0.15), effectiveBgColor)
                : Color.alphaBlend(Colors.white.withValues(alpha: 0.1), effectiveBgColor),
            effectiveBgColor,
          ],
        ) : null,
        ...
        border: Border.all(
          ...
          width: ResponsiveUtils.value(_isHovered && widget.isInteractive ? 1.5 : 1),
        ),
        boxShadow: (widget.elevation > 0 || _isHovered) && widget.isInteractive ? [
          BoxShadow(
            color: colorScheme.shadow.withValues(alpha: (_isHovered ? 0.15 : 0.1) * shadowStrength),
            blurRadius: ResponsiveUtils.value((widget.elevation + (_isHovered ? 4 : 0)) * 2),
            offset: Offset(0, ResponsiveUtils.value(widget.elevation + (_isHovered ? 2 : 0))),
          ),
        ] : null,
      ),
      child: ClipRRect(
