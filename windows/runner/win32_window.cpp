...
bool Win32Window::Create(const std::wstring& title,
  ...
  // Calculate window position (top-right, avoiding taskbar)
  int window_x = work_area.right - Scale(size.width, scale_factor);
  ...
  // Set window transparency and click-through
  SetLayeredWindowAttributes(window, RGB(0, 0, 0), 255, LWA_ALPHA);
  
  // Set window as transparent background
  DWM_BLURBEHIND bb = {0};
  ...
  // Set window as tool window (not shown in taskbar)
  LONG_PTR exStyle = GetWindowLongPtr(window, GWL_EXSTYLE);
  SetWindowLongPtr(window, GWL_EXSTYLE, exStyle | WS_EX_TOOLWINDOW);
